...
class chatgpt
{
    public $apikey, $url;
    //构造函数
    public function __construct($apikey, $url = false)
    {
        $this->apikey = $apikey;
        $this->url = !empty($url) ? $url : 'https://api.openai.com';
    }
    public function chat_once($prompts, $stream = false)
    {
    ...
    public function generate_image($prompt, $n, $size = "256x256", $response_format = "url")
    {
        $url = $this->url . '/v1/images/generations';
        $addheader = ['Accept: application/json', 'Content-Type: application/json', 'Authorization: Bearer ' . $this->apikey];
        $post = ['prompt' => $prompt, 'n' => $n, 'size' => $size, 'response_format' => $response_format];
        $data = $this->get_curl($url, json_encode($post), $addheader);
        //data b64_json或者url
        return $data;
    }
    //curl语句
